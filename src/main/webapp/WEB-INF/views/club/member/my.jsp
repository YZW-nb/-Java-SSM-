...
<div class="flex h-screen overflow-hidden">
    ...
    <div class="flex-1 flex flex-col overflow-hidden">
        ...
        <main class="flex-1 overflow-y-auto p-6 bg-gray-50">
            <div class="mb-6">
                <h2 class="text-[clamp(1.5rem,3vw,2rem)] font-bold text-gray-800">我的社团</h2>
                <p class="text-gray-600">您已加入的社团列表</p>
            </div>
            ...
            <div class="bg-white rounded-xl p-6 card-shadow">
                ...
                <c:if test="${empty myClubIds}">
                    <div class="py-12 text-center">
                        <div class="inline-flex items-center justify-center w-16 h-16 rounded-full bg-gray-100 mb-4">
                            <i class="fa fa-university text-gray-400 text-2xl"></i>
                        </div>
                        <h3 class="text-xl font-medium text-gray-900 mb-2">您还没有加入任何社团</h3>
                        <p class="text-gray-500 mb-6">加入社团可以参与丰富的校园活动，结交志同道合的朋友</p>
                        <a href="${pageContext.request.contextPath}/club/list" class="inline-flex items-center px-6 py-3 border border-transparent text-base font-medium rounded-lg shadow-sm text-white bg-primary hover:bg-primary/90 transition-colors">
                            <i class="fa fa-plus-circle mr-2"></i> 浏览社团
                        </a>
                    </div>
                </c:if>

                <c:if test="${not empty myClubIds}">
                    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                        <c:forEach items="${myClubIds}" var="clubId">
                            <div class="border border-gray-200 rounded-xl overflow-hidden hover:shadow-md transition-shadow">
                                <div class="h-40 bg-primary/10 relative">
                                    <img src="https://picsum.photos/id/${clubId}/600/400" alt="社团封面" class="w-full h-full object-cover">
                                    <div class="absolute top-3 right-3 bg-white rounded-full px-3 py-1 text-xs font-medium text-primary">
                                        成员
                                    </div>
                                </div>
                                <div class="p-4">
                                    <h4 class="font-bold text-gray-800">社团 ${clubId}</h4>
                                    <p class="text-gray-600 text-sm mt-1">这是一个非常活跃的社团</p>
                                    <div class="flex items-center mt-3 text-gray-500 text-sm">
                                        <span class="flex items-center mr-4">
                                            <i class="fa fa-users mr-1"></i> 56 成员
                                        </span>
                                        <span class="flex items-center">
                                            <i class="fa fa-calendar mr-1"></i> 成立于 2024-09-01
                                        </span>
                                    </div>
                                    <div class="flex items-center justify-between mt-4">
                                        <a href="${pageContext.request.contextPath}/club/member/list/${clubId}" class="px-4 py-2 bg-primary text-white rounded-lg hover:bg-primary/90 transition-colors">
                                            查看成员
                                        </a>
                                        <a href="${pageContext.request.contextPath}/club/member/exit/${clubId}" class="px-4 py-2 border border-gray-300 rounded-lg text-gray-700 hover:bg-gray-50 transition-colors">
                                            退出
                                        </a>
                                    </div>
                                </div>
