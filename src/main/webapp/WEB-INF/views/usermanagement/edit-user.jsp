...
<div class="flex h-screen overflow-hidden">
  ...
  <aside id="sidebar" class="w-64 bg-white shadow-lg transition-all duration-300 ease-in-out transform-gpu z-20">
    ...
    <div class="p-4">
      ...
      <nav class="space-y-1">
        ...
        <a href="${pageContext.request.contextPath}/admin/users/user/edit/${student.id}" class="flex items-center space-x-3 px-4 py-3 rounded-lg nav-active">
          <i class="fa fa-user-edit w-5 text-center"></i>
          <span>编辑用户</span>
        </a>
  ...
  <div class="flex-1 flex flex-col overflow-hidden">
    ...
    <main class="flex-1 overflow-y-auto p-6 bg-gray-50">
      <div class="mb-6">
        <h2 class="text-[clamp(1.5rem,3vw,2rem)] font-bold text-gray-800">编辑普通用户</h2>
        <p class="text-gray-600">编辑用户 <span class="font-medium">${student.username}</span> 的信息</p>
      </div>
      ...
      <div class="bg-white rounded-xl p-6 card-shadow max-w-2xl mx-auto">
        <div class="mb-6">
          ...
          <p class="text-gray-600">修改以下信息更新用户资料</p>
        </div>

        <form method="post" action="${pageContext.request.contextPath}/admin/users/user/edit/${student.id}">
            ...
            <div class="grid grid-cols-1 gap-4">
              ...
              <div>
                ...
                <input type="password" id="password" name="password" value="${student.password}" required
                       class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-primary focus:border-primary transition-colors">
                <p class="mt-1 text-sm text-gray-500">如果不修改密码，请保持当前值</p>
              </div>
