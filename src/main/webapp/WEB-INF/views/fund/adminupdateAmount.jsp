...
<div class="flex h-screen overflow-hidden">
    ...
    <aside id="sidebar" class="w-64 bg-white shadow-lg transition-all duration-300 ease-in-out transform-gpu z-20">
        ...
        <div class="p-4">
            <div class="flex items-center space-x-3 mb-6">
                <img src="https://picsum.photos/id/177/40/40" alt="用户头像" class="w-10 h-10 rounded-full object-cover border-2 border-primary">
                <div>
    ...
    <div class="flex-1 flex flex-col overflow-hidden">
        ...
        <main class="flex-1 overflow-y-auto p-6 bg-gray-50">
            <div class="mb-6">
                <h2 class="text-[clamp(1.5rem,3vw,2rem)] font-bold text-gray-800">修改金额</h2>
                <p class="text-gray-600">修改经费记录 #${fund.id} 的金额</p>
            </div>

            <!-- 经费信息预览 -->
            <div class="bg-white rounded-xl p-6 card-shadow mb-8">
                <h3 class="text-lg font-semibold text-gray-800 mb-4">经费信息</h3>
                <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
                    <div>
                        <h4 class="text-sm font-medium text-gray-500 mb-1">社团</h4>
                        <p class="text-gray-900">${clubName}</p>
                    </div>
                    <div>
                    ...
                    <div>
                        <h4 class="text-sm font-medium text-gray-500 mb-1">当前金额</h4>
                        <p class="text-lg font-bold ${fund.type == 1 ? 'text-green-600' : 'text-red-600'}">
                    ...
                    <div>
                        <h4 class="text-sm font-medium text-gray-500 mb-1">来源/用途</h4>
                        <p class="text-gray-900">${fund.sourceOrPurpose}</p>
                    </div>
                </div>
            </div>
            ...
            <!-- 修改金额表单 -->
            <div class="bg-white rounded-xl p-6 card-shadow mb-8">
                <form action="${pageContext.request.contextPath}/fund/admin/update/amount" method="post">
                    <!-- 确保表单中有这个隐藏字段 -->
                    <input type="hidden" name="id" value="${fund.id}">
                    <div class="mb-6">
                        <label class="block text-sm font-medium text-gray-700 mb-1">新金额（元） <span class="text-red-500">*</span></label>
                        <div class="flex items-center">
                            ...
                            <input type="number" name="newAmount" step="0.01" min="0.01" value="${fund.amount}" required class="flex-1 min-w-0 block w-full px-3 py-2 rounded-r-lg border border-gray-300 focus:outline-none focus:ring-2 focus:ring-primary/50 focus:border-primary" placeholder="请输入新金额">
                        </div>
                    </div>

                    <div class="mb-6">
                        <label class="block text-sm font-medium text-gray-700 mb-1">修改原因 <span class="text-red-500">*</span></label>
                        <textarea name="reason" rows="3" required class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-primary/50 focus:border-primary" placeholder="请输入修改金额的原因"></textarea>
                    </div>

                    <div class="flex justify-end space-x-3">
                        <a href="${pageContext.request.contextPath}/fund/admin/update/amount" class="px-4 py-2 border border-gray-300 rounded-lg text-sm font-medium text-gray-700 hover:bg-gray-50 transition-colors">
                            返回
                        </a>
                        <button type="submit" class="px-4 py-2 bg-accent text-white rounded-lg text-sm font-medium hover:bg-accent/90 transition-colors">
